...
Note all URLs should be relative to the `BASE_URL` environment variable, which is `/` if not defined.
...
Prize amounts *may* get stripped up `$` characters if the implementers are feeling like handling errors that day.
...
If an email has already been mapped to a prize, it is reported as a duplicate, but accepted, just like previously.

Prize names that cannot be mapped are reported the same way as in the [other format](#emails-and-prizes).

As above, the "Prize ID" column can be given, behaving the same way as in the [other format](#emails-and-prizes).

## 3. Admins choose to send emails
This will send an individualized email to every email that won a prize
with a personalized link (`/form/{some-unique-id}`) to open a portal where the winner can enter their details.
...
It should load the data from the spreadsheet (never from the DB copy)[^2] to populate the form with existing data
...
[^1]: The reasoning behind waiting until now is that it serves as a way to verify the email is valid before needing to share.

[^2]: Yes, it will be slow to load the page. No, I don't think it's worth adding complex cache invalidation.
      I suppose you could check the last updated time on the sheet, but honestly Google will send you that timestamp and all the data in about the same amount of time.
...
The form should continue syncing to the sheet in the same way, and create a new version of the filled documents
(read: *update*[^3], NOT delete and recreate; see [revisions docs](https://developers.google.com/workspace/drive/api/guides/manage-revisions) for more).
...
[^3]: The fascination with this is because we can view old versions. 
      ...
      we never lose data due to a mistake.
      The sheet changes are tracked, the document revisions are tracked, and it's all undoable if needed.